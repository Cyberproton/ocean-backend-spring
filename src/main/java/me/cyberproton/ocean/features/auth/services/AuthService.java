...
public class AuthService {
    ...
    public LoginResponse register(RegisterRequest registerRequest) {
        ...
        if (username != null) {
            ...
            if (isUsernameConflict) {
                throw new ResponseStatusException(
                        ...
                        "Failed to generate username from email %s".formatted(registerRequest.getEmail())
                );
            }
        }
        Set<Role> roles = Set.of(roleRepository.findByName("USER").orElseThrow(
        ...
        emailService.sendEmailUsingTemplate(
                EmailTemplateRequest.builder()
                                    .to(user.getEmail())
                                    .subject(messageSource.getMessage("welcome.subject", null,
                                                                      LocaleContextHolder.getLocale()
                                    ))
                                    .template(EmailTemplates.WELCOME)
                                    .model(Map.of(
                                            "recipientName",
                                            Objects.requireNonNullElse(user.getUsername(), user.getEmail())
                                    ))
                                    .build()
        );
        String token = jwtService.generateToken(user.getUsername());
        String refreshToken = jwtService.generateRefreshToken(user.getUsername());
        return LoginResponse.builder().accessToken(token).refreshToken(refreshToken).user(UserResponse.fromUser(user))
                            .build();
    }
    ...
    public LoginResponse login(LoginRequest loginRequest) {
        ...
        return LoginResponse.builder().accessToken(accessToken).refreshToken(refreshToken)
                            .user(UserResponse.fromUser(user)).build();
    }
    ...
    public RefreshTokenResponse refreshToken(RefreshTokenRequest refreshTokenRequest) {
        ...
        String accessToken = jwtService.generateToken(user.getEmail());
        String refreshToken = jwtService.generateRefreshToken(user.getEmail());
        return RefreshTokenResponse.builder().accessToken(accessToken).refreshToken(refreshToken)
                                   .user(UserResponse.fromUser(user)).build();
    }
    ...
    public ResetPasswordResponse requestResetPassword(ResetPasswordRequest resetPasswordRequest) {
        ...
        latestTokenOpt.ifPresent((latestToken) -> {
            if (latestToken.getCreatedAt().toInstant().plusMillis(tokenConfig.passwordReset().intervalInMilliseconds())
                           .isAfter(Instant.now())) {
                throw new ResponseStatusException(
                        HttpStatus.BAD_REQUEST,
                        "Please wait for %s seconds before requesting another reset password".formatted(
                                tokenConfig.passwordReset().intervalInMilliseconds() / 1000)
                );
            }
        });
        ...
        emailService.sendEmailUsingTemplate(
                EmailTemplateRequest.builder()
                                    ...
                                    .model(Map.of(
                                            ...
                                            "expiryInMinutes", tokenConfig.passwordReset()
                                                                          .maxAgeInMilliseconds() / 60000
                                    ))
                                    .build()
        );
    ...
    public RequestVerifyEmailResponse requestVerifyEmail(User user) {
        ...
        latestTokenOpt.ifPresent((latestToken) -> {
            if (latestToken.getCreatedAt().toInstant()
                           .plusMillis(tokenConfig.emailVerification().intervalInMilliseconds())
                           .isAfter(Instant.now())) {
                throw new ResponseStatusException(
                        HttpStatus.BAD_REQUEST,
                        "Please wait for %s seconds before requesting another verify email".formatted(
                                tokenConfig.emailVerification().intervalInMilliseconds() / 1000)
                );
            }
        });
        ...
        emailService.sendEmailUsingTemplate(
                EmailTemplateRequest.builder()
                                    .to(user.getEmail())
                                    .subject(messageSource.getMessage(
                                            "email.verification.subject", null,
                                            LocaleContextHolder.getLocale()
                                    ))
